...
final class EZEV_Core_Admin {
    ...
    public static function add_station(): void {
        ...
        global $wpdb;
        $orgs = $wpdb->get_results("SELECT organization_id,name FROM " . EZEV_Core_DB::table('organizations') . " WHERE status='active' ORDER BY name", ARRAY_A) ?: [];
        $sites = $wpdb->get_results("SELECT site_id,name,organization_ref FROM " . EZEV_Core_DB::table('sites') . " WHERE status='active' ORDER BY name", ARRAY_A) ?: [];
        ...
        <form class="ezev-station-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
            ...
            <div class="ezev-station-editor">
                <section class="ezev-panel ezev-form-panel">
                    ...
                    <div class="ezev-fields-2"><label>Organization<select name="organization_id"><option value="">Unassigned / Public Network</option><?php foreach ($orgs as $o): ?><option value="<?php echo esc_attr($o['organization_id']); ?>" <?php selected((string)($station['organization_id'] ?? ''),(string)$o['organization_id']); ?>><?php echo esc_html($o['name']); ?></option><?php endforeach; ?></select></label><label>Site<select name="site_id"><option value="">No site</option><?php foreach ($sites as $s): ?><option value="<?php echo esc_attr($s['site_id']); ?>" <?php selected((string)($station['site_id'] ?? ''),(string)$s['site_id']); ?>><?php echo esc_html($s['name']); ?></option><?php endforeach; ?></select></label></div>
    ...
    public static function organizations(): void {
        ...
        $sites = $wpdb->get_results("SELECT s.*, o.name AS organization_name FROM " . EZEV_Core_DB::table('sites') . " s LEFT JOIN " . EZEV_Core_DB::table('organizations') . " o ON o.organization_id=s.organization_ref ORDER BY s.name", ARRAY_A) ?: [];
        ...
        <div class="ezev-panel-grid ezev-panel-grid-2">
            <section class="ezev-panel"><span class="ezev-eyebrow">NEW ORGANIZATION</span><h2>Create workspace</h2><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><?php wp_nonce_field('ezev_create_org'); ?><input type="hidden" name="action" value="ezev_create_org"><label>Organization ID<input name="organization_id" required placeholder="EZEV-ORG-0001"></label><label>Name<input name="name" required placeholder="ABC Logistics"></label><div class="ezev-fields-2"><label>Type<select name="type"><option value="business">Business Customer</option><option value="partner">Partner</option><option value="investor">Investor</option></select></label><label>Country<select name="country_code"><option value="VN">Vietnam</option><option value="PH">Philippines</option><option value="CN">China</option></select></label></div><button class="ezev-button ezev-button-primary">Create organization</button></form></section>
            <section class="ezev-panel"><span class="ezev-eyebrow">NEW SITE</span><h2>Add physical location</h2><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><?php wp_nonce_field('ezev_create_site'); ?><input type="hidden" name="action" value="ezev_create_site"><label>Organization<select name="organization_id" required><option value="">Select</option><?php foreach($orgs as $o): ?><option value="<?php echo esc_attr($o['organization_id']); ?>"><?php echo esc_html($o['name']); ?></option><?php endforeach; ?></select></label><label>Site ID<input name="site_id" required placeholder="EZEV-SITE-HN-01"></label><label>Site Name<input name="name" required placeholder="Hanoi Warehouse"></label><label>Address<input name="address"></label><button class="ezev-button ezev-button-primary">Create site</button></form></section>
        </div>
        <section class="ezev-panel"><div class="ezev-panel-head"><div><span class="ezev-eyebrow">WORKSPACES</span><h2>Organizations</h2></div><span class="ezev-badge active"><?php echo count($orgs); ?> organizations</span></div><div class="ezev-card-grid"><?php foreach($orgs as $o): ?><div class="ezev-org-card"><span class="ezev-badge"><?php echo esc_html(strtoupper($o['type'])); ?></span><h3><?php echo esc_html($o['name']); ?></h3><p><?php echo esc_html($o['organization_id'].' · '.$o['country_code']); ?></p><small><?php echo esc_html($o['status']); ?></small></div><?php endforeach; ?><?php if(!$orgs): ?><p>No organizations yet.</p><?php endif; ?></div></section>
        <section class="ezev-panel"><span class="ezev-eyebrow">PHYSICAL LOCATIONS</span><h2>Sites</h2><div class="ezev-table-scroll"><table class="ezev-table"><thead><tr><th>Site</th><th>Organization</th><th>Address</th><th>Status</th></tr></thead><tbody><?php foreach($sites as $s): ?><tr><td><strong><?php echo esc_html($s['name']); ?></strong><small><?php echo esc_html($s['site_id']); ?></small></td><td><?php echo esc_html($s['organization_name'] ?: '—'); ?></td><td><?php echo esc_html($s['address']); ?></td><td><span class="ezev-badge active"><?php echo esc_html($s['status']); ?></span></td></tr><?php endforeach; ?></tbody></table></div></section>
        <?php self::footer();
    }
    ...
    public static function handle_create_org(): void {
        ...
        $organization_id=EZEV_Core_Domain::normalize_id((string)wp_unslash($_POST['organization_id']??''));
        $wpdb->insert(EZEV_Core_DB::table('organizations'),['organization_id'=>$organization_id,'org_code'=>$organization_id,'name'=>sanitize_text_field(wp_unslash($_POST['name']??'')),'type'=>sanitize_key($_POST['type']??'business'),'country_code'=>sanitize_text_field(wp_unslash($_POST['country_code']??'')),'status'=>'active','created_at'=>$now,'updated_at'=>$now]);
        EZEV_Core_DB::log('organization_created','organization',$organization_id); wp_safe_redirect(admin_url('admin.php?page=ezev-organizations&ezev_notice=saved')); exit;
    }
    ...
    public static function handle_assign_member(): void {
        ...
        $organization_id=EZEV_Core_Domain::normalize_id((string)wp_unslash($_POST['organization_id']??''));$org=EZEV_Core_Domain::organization_by_id($organization_id);if(!$org){wp_die('Invalid organization.');}$user_id=absint($_POST['user_id']??0);$existing=$wpdb->get_row($wpdb->prepare('SELECT * FROM '.EZEV_Core_DB::table('org_members').' WHERE organization_id=%d AND user_id=%d',(int)$org['id'],$user_id),ARRAY_A);$membership_id=$existing['membership_id']??EZEV_Core_Domain::new_id('membership');
        $membership=['membership_id'=>$membership_id,'organization_id'=>(int)$org['id'],'organization_ref'=>$organization_id,'user_id'=>$user_id,'role_key'=>sanitize_key($_POST['role_key']??'viewer'),'status'=>'active','created_at'=>$existing['created_at']??$now,'updated_at'=>$now];if($existing){$membership['id']=(int)$existing['id'];}
        $wpdb->replace(EZEV_Core_DB::table('org_members'),$membership);
        EZEV_Core_DB::log('membership_assigned','membership',$membership_id); wp_safe_redirect(admin_url('admin.php?page=ezev-access&ezev_notice=saved')); exit;
    }
