...
final class EZEV_Operations_REST {
    ...
    public static function can_view(): bool|WP_Error {
        ...
        if (current_user_can('manage_options') || current_user_can('ezev_view_operations') || current_user_can('ezev_view_internal')) {
            return true;
        }
    ...
    private static function allowed_station_keys(): ?array {
        ...
        $post_ids=EZEV_Core_Auth::allowed_station_post_ids(get_current_user_id());$keys=[];
    ...
    public static function overview(): WP_REST_Response {
        $p=EZEV_Operations_Provider_Manager::active();$chargers=self::filter_rows($p->fetch_chargers());$connectors=self::filter_rows($p->fetch_connectors());$sessions=self::filter_rows($p->fetch_sessions());$energy=self::filter_rows($p->fetch_energy());$alerts=self::filter_rows($p->fetch_alerts());$today=gmdate('Y-m-d');
        $data=['chargers_total'=>count($chargers),'chargers_available'=>count(array_filter($chargers,fn($r)=>($r['status']??'')==='available')),'chargers_charging'=>count(array_filter($chargers,fn($r)=>($r['status']??'')==='charging')),'chargers_faulted'=>count(array_filter($chargers,fn($r)=>($r['status']??'')==='faulted')),'connectors_total'=>count($connectors),'current_power'=>array_sum(array_map(fn($r)=>(float)($r['current_power_kw']??0),$chargers)),'energy_today'=>array_sum(array_map(fn($r)=>str_starts_with((string)($r['recorded_at']??''),$today)?(float)($r['ev_kwh']??0):0,$energy)),'sessions_today'=>count(array_filter($sessions,fn($r)=>str_starts_with((string)($r['started_at']??''),$today))),'open_alerts'=>count(array_filter($alerts,fn($r)=>($r['status']??'open')==='open'))];
    ...
    public static function connectors(): WP_REST_Response { return rest_ensure_response(['connectors'=>self::filter_rows(EZEV_Operations_Provider_Manager::active()->fetch_connectors())]); }
    ...
    public static function webhook(WP_REST_Request $request): WP_REST_Response|WP_Error {
        ...
        if($secret===''){ return new WP_Error('missing_secret','Webhook secret is not configured for this integration.',['status'=>401]); }
        $ts_header=(string)($request->get_header('x-ezev-timestamp')??'');
        if(!$ts_header||!is_numeric($ts_header)){ return new WP_Error('missing_timestamp','Missing or invalid timestamp header.',['status'=>401]); }
        $ts=(int)$ts_header;
        $diff=abs(time()-$ts);
        if($diff>300){ return new WP_Error('replay_rejected','Webhook timestamp out of acceptable window (replay protection).',['status'=>401]); }
        $raw=$request->get_body();
        $given=strtolower((string)$request->get_header('x-ezev-signature'));
        $expected=hash_hmac('sha256',$ts.'.'.$raw,$secret);
        if(!$given||!hash_equals($expected,$given)){ return new WP_Error('invalid_signature','Invalid webhook signature.',['status'=>401]); }
